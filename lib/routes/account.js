...
module.exports = function (
  ...
  var routes = [
    ...
    },
    {
      ...
      handler: function (request, reply) {
        log.begin('Account.web-sessions', request)
        var sessionToken = request.auth.credentials
        var uid = sessionToken.uid

        db.webSessions(uid).then(
          function (webSessions) {
            reply(webSessions.map(function (session) {
              var webSession = {
                id: session.tokenId.toString('hex'),
                userAgent: session.uaBrowser + ' ' + session.uaBrowserVersion,
                lastAccessTime: session.lastAccessTime,
                lastAccessTimeFormatted: ''
              }
              // combine returned UA, also handle NULLS
              // Unknown browser

              return butil.unbuffer(webSession)
            }))
          },
          reply
        )
